<?PHP
      ...
      function drawCompanyInformation($ReportName)
      {
        echo "<table align='center' border='0' cellspacing='0' cennpadding='0'>";
        echo "<tr><td align='center'><font size='4px'><b>Persona</b><font></td></tr>";
        echo "<tr><td align='center'><font size='2px'>House# 121 D, Read# 44, Gulshan# 2, Dhaka.</td></tr>";
        echo "<tr><td>&nbsp;</td></tr>";
      ...
      function drawMassage($Massage,$Event)
      {
            ...
            echo "<br><br><br>".
                 ...
                 "    <tr><td width='100%' height='26'><input class='Minput' type='button' value='Back' name='B3' style='float: right' $Event></td></tr>".
                 "</table>";
      }
      ...
      function GetComplainId($table_name,$FieldName)
      {
            $GetId="select CONCAT(SUBSTRING(YEAR(CURDATE()),3,4),
                 substring(curdate(),6,2),
                 substring(curdate(),9,2),
                 substring(curtime(),1,2),
                 substring(curtime(),4,2),
                 IFNULL(MAX(FLOOR(SUBSTRING($FieldName,11)))+1,'0')) as ID from $table_name where
                 substring($FieldName,1,2)=substring(YEAR(curdate()),3,4) AND
                 substring($FieldName,3,2)=substring(curdate(),6,2) AND
                 substring($FieldName,5,2)=substring(curdate(),9,2) AND
                 substring($FieldName,7,2)=substring(curtime(),1,2) AND
                 substring($FieldName,9,2)=substring(curtime(),4,2) ";

            $rs=mysql_query($GetId);
            $DataId=mysql_fetch_array($rs);
            $ComplainId=$DataId[ID];

            return  $ComplainId;
      }
	  function createNewCombo($ComboName,$TableName,$ID,$Name,$Condition,$selectedValue)
      {
                $query=" select
                                $ID as ID,
                                $Name as Name
      ...
      function createList($TableName,$ID,$Name,$Condition,$selectedValue)
      {
                $query=" select
                                $ID as ID,
                                $Name as Name
      ...
      function CreateQueryComboS($ComboName,$query,$FirstOption,$selectedValue)
      {
            $ResultSet= mysql_query($query)
                or die("Invalid query: " . mysql_error());
