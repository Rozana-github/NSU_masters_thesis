...
function copyRecords(costcodeid,costcentername,glcode,gldescripton,description,dr_cr_amount,trntype,Numrows)
{

        for(var l=0;l<Numrows;l++)
        {
            //alert(dr_cr_amount[l]+" "+trntype);
            ...
            if(trntype[l]=='Dr')
            {
                  DebitAmount[l]=dr_cr_amount[l];
                  CreditAmount[l]=0;
            }
            else if(trntype[l]=='Cr')
            {
                  DebitAmount[l]=0;
                  CreditAmount[l]=dr_cr_amount[l];
            }
        }
...
function DrawTable(start)
{
        ...
        if(DebitAmount[index]=='0' && CreditAmount[index]=='0')
        {
                alert('Value Required');
        }
        else if(DebitAmountLength[index]==0 ||CreditAmountLength[index]==0 )
        {
                alert('Value Required');
        }
        else
        {
                ...
                else
                {
                        ...
                        {
                        //alert(DebitAmount[i]);
                        //alert(CreditAmount[i]);
                        DataRow[i]='';
                        DataRow[i]+="<table border='1' align='center' width='90%' bordercolor='black' cellpadding='0' cellspacing='0'>";
                        DataRow[i]+="<tr>";
                        DataRow[i]+="<td  width='20%' align='center' class='td_e'><input type='text' style='border:0;cursor:hand' Size='15' name='txtcostcenter["+i+"]'  value='"+costcenter_name[i]+"' readonly onclick='EditInformation("+i+");' class='input_e'><input type='hidden' value='"+costcenter[i]+"' name='cmbCostCenter1["+i+"]'></td>";
                        DataRow[i]+="<td  width='30%'  class='td_e'><input type='text' style='border:0;cursor:hand;' name='txtChartofAccount["+i+"]'  value='"+ChartofAccountValue[i]+"' readonly onclick='EditInformation("+i+");'><input type='hidden' value='"+glcode[i]+"' name='txtglcode["+i+"]'></td>";
                        DataRow[i]+="<td  width='20%' class='td_e'><input type='text' style='border:0;cursor:hand;' size='20' name='txtRemarks["+i+"]'  value='"+remarks[i]+"' readonly onclick='EditInformation("+i+");')></td>";
                        DataRow[i]+="<td  width='10%' class='td_e'><input type='text' style='border:0;cursor:hand;text-align: right' size='5' name='txtDebitAmount["+i+"]'  value='"+DebitAmount[i]+"' readonly onclick='EditInformation("+i+");'></td>";
                        DataRow[i]+="<td  width='10%' class='td_e'><input type='text' style='border:0;cursor:hand;text-align: right' size='5' name='txtCreditAmount["+i+"]' value='"+CreditAmount[i]+"' readonly onclick='EditInformation("+i+");'></td>";
                        DataRow[i]+="<td  width='10%' class='forms_button_e'><input type='button' style='border:0;cursor:hand;' class='forms_button_e' value='Delete' name='del["+i+"]' onClick='DeleteDataRow("+i+")'></td>";
                        DataRow[i]+="</tr></table>";
...
<form name='frmVoucherEntry' method='post' action='SaveeditMasTrnJournal.php'>
                                ...
                                <?PHP
                                        $party_sql="select
                                                        ...
                                                        order by
                                                                Company_Name
                                                ";
                                        createQueryCombo("Supplier",$party_sql,'-1',$supplierid);
                                ?>
                       </select>
                </td>
...
<script language='JavaScript'>
        ...
        var dr_cr_amount=new Array(100);
        var trntype=new Array(100);
        var Numrows=0;
...
<?PHP
        ...
        $query="SELECT
                ...
                FROM
                        ...
                        left join mas_cost_center on mas_cost_center.cost_code=trn_journal.cost_code
                WHERE
        ...
        $j=0;
            while($row=mysql_fetch_array($result))
            {
                  ...
                  echo   "dr_cr_amount[$j]=\"$amount\";";
                  echo   "trntype[$j]=\"$ttype\";";
                  $j++;
            }
         echo "Numrows=$j;\n";
         echo "copyRecords(costcodeid,costcentername,glcode,gldescripton,description,dr_cr_amount,trntype,Numrows);";
