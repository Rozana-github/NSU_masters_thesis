<?PHP
      include "Library/dbconnect.php";
      include "Library/Library.php";
      include "Library/SessionValidate.php";
?>
<?PHP
      mysql_query("BEGIN") or die("Begin Error.");
?>
<html>
      <head>
            <meta http-equiv='Content-Type' content='text/html; charset=windows-1252'>
            <title>Invoice Posting</title>
            <LINK href="Style/generic_form.css" type='text/css' rel='stylesheet'>
            <link rel='stylesheet' type='text/css' href='Style/eng_form.css' />
      </head>
      <body class='body_e'>
            <?PHP
                  $query="INSERT INTO mas_journal
                          (
                              journalno,
                              journaltype,
                              journaldate,
                              partyid,
                              remarks,
                              journal_status,
                              entry_by,
                              entry_date
                          )
                          VALUES
                          (
                              '$VoucherNo',
                              'JV',
                              STR_TO_DATE('$cboVoucherDay-$cboVoucherMonth-$cboVoucherYear','%e-%c-%Y'),
                              '$CustomerID',
                              '$Remarks',
                              '0',
                              '$SUserID',
                              DATE(now())
                          )";
                  echo $query."<br>";
                  mysql_query($query) or die("Error: 1".mysql_error());
                  
                  $query="select last_insert_id() as journalid from mas_journal";
                  $rs=mysql_query($query) or die("Error: 2".mysql_error());
                  while($row=mysql_fetch_array($rs))
                  {
                        extract($row);
                  }

                  $i=1;
                  $querydebtorstype="select
                                                        customer_id,
                                                        type
                                                from
                                                        mas_customer
                                                where
                                                        customer_id='$CustomerID'
                                                ";
                               //echo $querydebtorstype;
                               $rsdebtorstype=mysql_query($querydebtorstype)or die(mysql_error());
                               while($row=mysql_fetch_array($rsdebtorstype))
                               {
                                        extract($row);
                               }
                  
                     if($txtTotalAITAmount!='')
                     {
                     if($type==1)
                     {
                        $query="INSERT INTO trn_journal
                                    (journalid,slno,glcode,amount,ttype)
                                VALUES
                                    ('$journalid','$i','10401','$txtTotalAITAmount','Dr')";
                        echo $query."<br>";
                        mysql_query($query) or die("Error: 3".mysql_error());
                     }
                     if($type==2)
                     {
                       $query="INSERT INTO trn_journal
                                    (journalid,slno,glcode,amount,ttype)
                                VALUES
                                    ('$journalid','$i','10501','$txtTotalAITAmount','Dr')";
                                    echo $query."<br>";
                        mysql_query($query) or die("Error: 3".mysql_error());
                     }

                        $i++;
                  
                        $query="INSERT INTO trn_journal
                                    (journalid,slno,glcode,amount,ttype)
                                VALUES
                                    ('$journalid','$i','10615','$txtTotalAITAmount','Cr')";
                                    echo $query."<br>";
                        mysql_query($query) or die("Error: 4".mysql_error());
                    }
                    $i++;
                    if($txtTotalSalesAmount!='')
                    {
                       if($type==1)
                     {
                        $query="INSERT INTO trn_journal
                                    (journalid,slno,glcode,amount,ttype)
                                VALUES
                                    ('$journalid','$i','10401','$txtTotalSalesAmount','Cr')";
                        echo $query."<br>";
                        mysql_query($query) or die("Error: 3".mysql_error());
                     }
                     if($type==2)
                     {
                       $query="INSERT INTO trn_journal
                                    (journalid,slno,glcode,amount,ttype)
                                VALUES
                                    ('$journalid','$i','10501','$txtTotalSalesAmount','Cr')";
                                    echo $query."<br>";
                        mysql_query($query) or die("Error: 3".mysql_error());
                     }

                        $i++;

                        $query="INSERT INTO trn_journal
                                    (journalid,slno,glcode,amount,ttype)
                                VALUES
                                    ('$journalid','$i','30201','$txtTotalSalesAmount','Dr')";
                                    echo $query."<br>";
                        mysql_query($query) or die("Error: 4".mysql_error());
                    }

                  
                  for($i=0;$i<$hidIndex;$i++)
                  {
                        if($chkAccept[$i]=="ON")
                        {
                              $query="INSERT INTO mas_invoice_collection
                                    (
                                          invoiceobjet_id,
                                          journal_id,
                                          collection_date,
                                          customer_id,
                                          ait_amount,
                                          sales_return_amount,
                                          entry_by,
                                          entry_date
                                    )
                                    VALUES
                                    (
                                          '".$txtInvoiceObjectID[$i]."',
                                          '$journalid',
                                          STR_TO_DATE('$cboVoucherDay-$cboVoucherMonth-$cboVoucherYear','%e-%c-%Y'),
                                          '$CustomerID',
                                          '".$txtAITAmount[$i]."',
                                          '".$txtSalesAmount[$i]."',
                                          '$SUserID',
                                          DATE(now())
                                    )";
                              echo $query."<br>";
                              mysql_query($query) or die("Error: Collection: ".mysql_error());
                              
                              /*if(($txtReceivedAmount[$i]+$txtAmount[$i])>=$txtNetBill[$i])
                              {
                                    $query="update mas_invoice set receive_status='2' where invoiceobjet_id='".$txtInvoiceObjectID[$i]."'";
                                    mysql_query($query) or die("Error: Invoice Status: ".mysql_error());
                              }
                              else
                              {
                                    $query="update mas_invoice set receive_status='1' where invoiceobjet_id='".$txtInvoiceObjectID[$i]."'";
                                    mysql_query($query) or die("Error: Invoice Status: ".mysql_error());
                              }*/
                        }
                  }
                  

                  $query="update mas_latestjournalnumber set JV='$VoucherNo'";
                  echo $query."<br>";
                  mysql_query($query) or die("Error: 7".mysql_error());
                  
                  drawNormalMassage("Data Saved Successfully");
            ?>
      </body>
</html>
<?PHP
      mysql_query("COMMIT") or die("Commit Error.");
      mysql_close();
?>
